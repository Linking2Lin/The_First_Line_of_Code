...
### 协程（可以看作轻量级线程）
协程：一种全新的处理并发的方式，可以在语言层面实现不同协程间的切换，在单线程模式下模拟出多线程的效果，代码执行时的挂起与恢复由语言来控制，而线程需要由操作系统来调度
特性：
* 轻量：可以在单个线程上运行多个线程，因为协程可以挂起，不会阻塞当前的线程
* 内存泄露少：结构化并发机制在一个作用域内执行多个操作
* 内置取消支持：取消功能会自动通过正在运行的协程层次结构传播
* Jetpack集成：许多Jetpack库都提供全面协程支持的扩展
基本概念：
* suspend function：挂起函数，不会阻塞其所在的线程，而是将协程挂起，在特定的时候再恢复执行
* CoroutineScope：协程作用域，用来指定协程的作用范围，可以来管理多个协程的生命周期，所有协程都要通过CoroutineScope来启动
* CoroutineContext：协程上下文，包含多种类型的配置参数
