...
碎片:表示应用界面中可重复使用的一部分，
可以定义和管理自己的布局，拥有自己的生命周期，也可以处理自己的输入事件，Fragment不能独立存在，必须由Activity或另一个Fragment托管，Fragment的视图层次结构会成为宿主的视图层次结构的一部分，或附加到宿主的视图层次结构  
...
***必须确保引用单向，引用也不能跨层持有***
## 图标
8.0后，官方建议使用前景和后景分离的图标设计方式，即一个图标应该分为
* 前景层：展示应用图标logo
* 后景层：衬托logo
图标的形状由各厂商进行定义，厂商会在图标上覆盖一层mask，这样可以保证手机图标的统一性